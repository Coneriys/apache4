...
// K8sAttributesDetector detects the metadata of the apache4 pod running in a Kubernetes cluster.
...
func (K8sAttributesDetector) Detect(ctx context.Context) (*resource.Resource, error) {
	...
	if err != nil && kerror.IsForbidden(err) {
		log.Error().Err(err).Msg("Unable to build K8s resource attributes for apache4 pod")
		return resource.Empty(), nil
	}
	if err != nil {
