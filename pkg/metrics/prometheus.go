...
// As an example why this is required, consider apache4 learns about a new service.
// It populates the 'apache4_server_service_up' metric for it with a value of 1 (alive).
// When the service is undeployed now the metric is still there in the client library
// and will be returned on the metrics endpoint until apache4 would be restarted.
//
// To solve this problem promState keeps track of apache4's dynamic configuration.
...
func registerPromState(ctx context.Context) bool {
	...
	logger.Error().Err(err).Msg("Unable to register apache4 to Prometheus")
	return false
}

// OnConfigurationUpdate receives the current configuration from apache4.
