...
func Test_buildConfiguration(t *testing.T) {
	testCases := []struct {
			...
			expected: &dynamic.Configuration{
				...
				HTTP: &dynamic.HTTPConfiguration{
					Routers: map[string]*dynamic.Router{
						"dev-Test": {
							Service:     "dev-Test",
							Rule:        "Host(`dev-Test.apache4.wtf`)",
							DefaultRule: true,
						},
					},
					Middlewares: map[string]*dynamic.Middleware{},
		...
		},
		{
			...
			constraints: `Tag("apache4.tags=bar")`,
			expected: &dynamic.Configuration{
				TCP: &dynamic.TCPConfiguration{
					Routers:           map[string]*dynamic.TCPRouter{},
					Middlewares:       map[string]*dynamic.TCPMiddleware{},
		...
		},
		{
			...
			constraints: `Tag("apache4.tags=foo")`,
			expected: &dynamic.Configuration{
				TCP: &dynamic.TCPConfiguration{
					Routers:           map[string]*dynamic.TCPRouter{},
					Middlewares:       map[string]*dynamic.TCPMiddleware{},
			...
			items: []itemData{
				{
					...
					Namespace:  "ns",
					Labels: map[string]string{
						"apache4.tcp.routers.test.rule": "HostSNI(`foobar`)",
					},
					Address: "127.0.0.1",
					Port:    "80",
					Status:  api.HealthPassing,
				},
				{
					...
					Labels: map[string]string{
						"apache4.consulcatalog.canary":         "true",
						"apache4.tcp.routers.test-canary.rule": "HostSNI(`canary.foobar`)",
					},
					Address: "127.0.0.2",
					Port:    "80",
					Status:  api.HealthPassing,
			...
			items: []itemData{
				{
					...
					Namespace:  "ns",
					Labels: map[string]string{
						"apache4.udp.routers.test.entrypoints": "udp",
					},
					Address: "127.0.0.1",
					Port:    "80",
					Status:  api.HealthPassing,
				},
				{
					...
					Labels: map[string]string{
						"apache4.consulcatalog.canary":                "true",
						"apache4.udp.routers.test-canary.entrypoints": "udp",
					},
					Address: "127.0.0.2",
					Port:    "80",
					Status:  api.HealthPassing,
			...
			items: []itemData{
				{
					...
					Namespace:  "ns",
					Labels: map[string]string{
						"apache4.udp.routers.test-udp-label.service":                           "test-udp-label-service",
						"apache4.udp.routers.test-udp-label.entryPoints":                       "udp",
						"apache4.udp.services.test-udp-label-service.loadBalancer.server.port": "21116",
					},
					Address: "127.0.0.1",
					Port:    "80",
					Status:  api.HealthPassing,
	...
	for _, test := range testCases {
		t.Run(test.desc, func(t *testing.T) {
			...
			config.SetDefaults()
			config.DefaultRule = "Host(`{{ normalize .Name }}.apache4.wtf`)"
