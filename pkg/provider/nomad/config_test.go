...
func Test_defaultRule(t *testing.T) {
	testCases := []struct {
		...
		},
		{
			...
			rule: `Host("{{ .Name }}.{{ index .Labels "apache4.domain" }}")`,
			expected: &dynamic.Configuration{
				TCP: &dynamic.TCPConfiguration{
					Routers:           map[string]*dynamic.TCPRouter{},
					Middlewares:       map[string]*dynamic.TCPMiddleware{},
...
func Test_buildConfig(t *testing.T) {
	testCases := []struct {
			...
			expected: &dynamic.Configuration{
				...
				HTTP: &dynamic.HTTPConfiguration{
					Routers: map[string]*dynamic.Router{
						"dev-Test": {
							Service:     "dev-Test",
							Rule:        "Host(`dev-Test.apache4.test`)",
							DefaultRule: true,
						},
					},
					Middlewares: map[string]*dynamic.Middleware{},
			...
			expected: &dynamic.Configuration{
				...
				HTTP: &dynamic.HTTPConfiguration{
					Routers: map[string]*dynamic.Router{
						"Test1": {
							Service:     "Test1",
							Rule:        "Host(`Test1.apache4.test`)",
							DefaultRule: true,
						},
						"Test2": {
							Service:     "Test2",
							Rule:        "Host(`Test2.apache4.test`)",
							DefaultRule: true,
						},
					},
					Middlewares: map[string]*dynamic.Middleware{},
		...
		{
			desc: "one service with rule label and one apache4 service",
			items: []item{
				{
					ID:   "id1",
					Name: "Test",
		...
		{
			desc: "one service with rule label and two apache4 services",
			items: []item{
				{
					ID:   "id1",
					Name: "Test",
		...
		{
			desc: "two services with same apache4 service and different passhostheader",
			items: []item{
				{
					ID:   "id1",
					Name: "Test",
		...
		{
			desc: "one service with apache4.enable false",
			items: []item{
				{
					ID:   "id1",
					Name: "Test",
	...
	for _, test := range testCases {
		t.Run(test.desc, func(t *testing.T) {
			p := new(Provider)
			p.SetDefaults()
			p.DefaultRule = "Host(`{{ normalize .Name }}.apache4.test`)"
