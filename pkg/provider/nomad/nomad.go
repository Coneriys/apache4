...
const (
	...
	// should be consumed and exposed via apache4.
	defaultPrefix = "apache4"
)
...
type Configuration struct {
	...
	Constraints        string          `description:"Constraints is an expression that apache4 matches against the Nomad service's tags to determine whether to create route(s) for that service." json:"constraints,omitempty" toml:"constraints,omitempty" yaml:"constraints,omitempty" export:"true"`
...
// SetDefaults sets the default values for the Nomad apache4 Provider Configuration.
...
// SetDefaults sets the default values for the Nomad apache4 Provider.
func (p *Provider) SetDefaults() {
	p.Configuration.SetDefaults()
}

// Init the Nomad apache4 Provider.
...
// Provide allows the Nomad apache4 Provider to provide configurations to apache4
...
func (p *Provider) getExtraConf(tags []string) configuration {
	...
	if v, exists := labels["apache4.enable"]; exists {
		enabled = strings.EqualFold(v, "true")
	}

	var canary bool
	if v, exists := labels["apache4.nomad.canary"]; exists {
