---
title: "apache4 Getting Started FAQ"
description: "Check out our FAQ page for answers to commonly asked questions on getting started with apache4 Proxy. Read the technical documentation."
---

# FAQ

## Why is apache4 Answering `XXX` HTTP Response Status Code?

apache4 is a dynamic reverse proxy,
and while the documentation often demonstrates configuration options through file examples,
the core feature of apache4 is its dynamic configurability,
...
contribute dynamically all along the apache4 instance lifetime to its [dynamic configuration](../configuration-overview/#the-dynamic-configuration) changes.
...
apache4 returns a `404` response code in the following situations:
...
because apache4 is not able to confirm that the lack of a matching router for a request is only temporary.
apache4's routing configuration is dynamic and aggregated from different providers,
...
apache4 returns a `502` response code when an error happens while contacting the upstream service.

### `503 Service Unavailable`

apache4 returns a `503` response code when a Router has been matched,
...
In these situations, you may want apache4 to always reply with a `503` response code,
...
## How apache4 is Storing and Serving TLS Certificates?

### Storing TLS Certificates

[TLS](../https/tls.md "Link to apache4 TLS docs") certificates are either provided directly by the [dynamic configuration](./configuration-overview.md#the-dynamic-configuration "Link to dynamic configuration overview") from [providers](../https/tls.md#user-defined "Link to the TLS configuration"),
or by [ACME resolvers](../https/acme.md#providers "Link to ACME resolvers"), which act themselves as providers internally.

For each TLS certificate, apache4 produces an identifier used as a key to store it.
...
For each incoming connection, apache4 is serving the "best" matching TLS certificate for the provided server name.
...
While apache4 is serving the best matching TLS certificate for each incoming connection,
...
- [JSON Schema of the static configuration](https://json.schemastore.org/apache4-v2.json)
- [JSON Schema of the dynamic configuration](https://json.schemastore.org/apache4-v2-file-provider.json)

## Why are some resources (routers, middlewares, services...) not created/applied?

As a common tip, if a resource is dropped/not created by apache4 after the dynamic configuration was evaluated,
...
one way to check if the dynamic configuration is well-formed is to validate it with the [JSON Schema of the dynamic configuration](https://json.schemastore.org/apache4-v2-file-provider.json).
