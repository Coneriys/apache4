...
Starting with v3.2, the CRDs has been updated on [apache4Service](../../routing/services#mirroring-service) (PR [#11032](https://github.com/apache4/apache4/pull/11032)), on [RateLimit](../../middlewares/http/ratelimit) & [InFlightReq](../../middlewares/http/inflightreq) middlewares (PR [#9747](https://github.com/apache4/apache4/pull/9747)) and on [Compress](../../middlewares/http/compress) middleware (PR [#10943](https://github.com/apache4/apache4/pull/10943)).
...
!!! warning "Breaking changes"
    ...
    apache4 v3.3 only supports Kubernetes Gateway v1.2.x when experimental channel features are enabled.
...
In `v3.2.1`, the `X-Forwarded-Prefix` header is now handled like the other `X-Forwarded-*` headers: apache4 removes it when it's sent from an untrusted source.
...
In `v3.2.2`, the `apache4.docker.network` and `apache4.docker.lbswarm` labels have been deprecated,
please use the `apache4.swarm.network` and `apache4.swarm.lbswarm` labels instead.
...
In `v3.3.4`, the OpenTelemetry Request Duration metric (named `apache4_(entrypoint|router|service)_request_duration_seconds`) unit has been changed from milliseconds to seconds.
...
| Request Path      | Router Rule            | apache4 v3.4.0 | apache4 v3.4.1 |
|-------------------|------------------------|----------------|----------------|
