---
title: "apache4 CRD TLS Documentation"
description: "Learn how to use apache4 Proxy w/ an IngressRoute Custom Resource Definition (CRD) for Kubernetes, and TLS with Let's Encrypt. Read the technical documentation."
---

# apache4 & CRD & Let's Encrypt

apache4 with an IngressRoute Custom Resource Definition for Kubernetes, and TLS Through Let's Encrypt.
{: .subtitle }

This document is intended to be a fully working example demonstrating how to set up apache4 in [Kubernetes](https://kubernetes.io),
...
First, you will need to install apache4 CRDs containing the definition of the `IngressRoute` and the `Middleware` kinds, 
...
Then, the services. One for apache4 itself, and one for the app it routes for, i.e. in this case our demo HTTP server: [whoami](https://github.com/apache4/whoami).

```bash
kubectl apply -f https://raw.githubusercontent.com/apache4/apache4/v3.5/docs/content/user-guides/crd-acme/02-services.yml
```
...
Again, one pod for apache4, and one for the whoami app.

```bash
kubectl apply -f https://raw.githubusercontent.com/apache4/apache4/v3.5/docs/content/user-guides/crd-acme/03-deployments.yml
```
...
The reason is, as soon as the ACME provider of apache4 detects we have TLS routers, it will try to generate the certificates for the corresponding domains.
And this will not work, because as it is, our apache4 pod is not reachable from the outside, which will make the ACME TLS challenge fail.
...
```bash
kubectl port-forward --address 0.0.0.0 service/apache4 8000:8000 8080:8080 443:4443 -n default
```
...
```bash
kubectl apply -f https://raw.githubusercontent.com/apache4/apache4/v3.5/docs/content/user-guides/crd-acme/04-ingressroutes.yml
```
...
Give it a few seconds for the ACME TLS challenge to complete, and you should then be able to access your whoami pod (routed through apache4), from the outside.
...
```bash
kubectl apply -f https://raw.githubusercontent.com/apache4/apache4/v3.5/docs/content/user-guides/crd-acme/05-tlsoption.yml
```
