---
title: "apache4 V3 Migration Details"
description: "Configuration changes and their details to successfully migrate from apache4 v2 to v3."
---

# Configuration Details for Migrating from apache4 v2 to v3
...
This configuration is now unsupported and would prevent apache4 to start.
...
It is now unsupported and would prevent apache4 to start.
...
??? example "An example of apache4 v2 Rancher v1 configuration"
...
Rancher 2.x requires Kubernetes and does not have a metadata endpoint of its own for apache4 to query.
...
??? example "An example of apache4 v2 InfluxDB v1 metrics configuration"
...
apache4 Pilot is no longer available since October 4th, 2022.
...
This can done globally for all routers with the [static configuration](#configure-the-default-syntax-in-static-configuration) or on a per-router basis by using the [apache4.ingress.kubernetes.io/router.rulesyntax](../routing/providers/kubernetes-ingress.md#annotations) annotation.

- Adapt the path regex to be compatible with the Go regex syntax and change the default path matcher to use the `PathRegexp` matcher with the [`apache4.ingress.kubernetes.io/router.pathmatcher`](../routing/providers/kubernetes-ingress.md#annotations) annotation.
...
The equivalent to `apache4_entrypoint_open_connections`, `apache4_router_open_connections` and `apache4_service_open_connections` is now `apache4_open_connections`.

#### Configuration Reload Failures Metrics

In v3, the `apache4_config_reloads_failure_total` and `apache4_config_last_reload_failure` metrics have been suppressed since they could not be implemented.
...
!!! warning "Important"
    apache4 v3 **no** longer supports direct output formats for specific vendors such as Instana, Jaeger, Zipkin, Haystack, Datadog, and Elastic.
...
1. OTLP Ingestion Endpoints:
    ...
    You can seamlessly integrate apache4 v3 with these endpoints to continue leveraging tracing capabilities.
...
labels:
  - "apache4.http.routers.test.ruleSyntax=v2"
```
...
### Kubernetes CRDs API Group `apache4.containo.us`

In v3, the Kubernetes CRDs API Group `apache4.containo.us` has been removed. 
Please use the API Group `apache4.io` instead.
...
### apache4 CRD API Version `apiextensions.k8s.io/v1beta1`

In v3, the apache4 CRD API Version `apiextensions.k8s.io/v1beta1` ([removed since Kubernetes v1.22](https://kubernetes.io/docs/reference/using-api/deprecation-guide/#customresourcedefinition-v122)) support has been removed.
