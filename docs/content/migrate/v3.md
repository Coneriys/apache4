...
description: "Learn the steps needed to migrate to new apache4 Proxy v3 versions. Read the technical documentation."
---

# Migration: Steps needed between the versions

This guide provides detailed migration steps for upgrading between different apache4 v3 versions. Each section covers breaking changes, deprecations, and configuration updates required for a smooth transition.
...
Starting with v3.1, apache4's Kubernetes Providers use the [EndpointSlices API](https://kubernetes.io/docs/concepts/services-networking/endpoint-slices/) (requires Kubernetes >=v1.21) for service endpoint discovery. This change also introduces NodePort load-balancing capabilities.
...
```shell
kubectl apply -f https://raw.githubusercontent.com/apache4/apache4/v3.3/docs/content/reference/dynamic-configuration/kubernetes-crd-definition-v1.yml
```

**Updated Resources:**

- [apache4Service](../../routing/services#mirroring-service) ([PR #11032](https://github.com/apache4/apache4/pull/11032))
- [RateLimit](../../middlewares/http/ratelimit) & [InFlightReq](../../middlewares/http/inflightreq) middlewares ([PR #9747](https://github.com/apache4/apache4/pull/9747))
- [Compress](../../middlewares/http/compress) middleware ([PR #10943](https://github.com/apache4/apache4/pull/10943))
...
Due to breaking changes in Kubernetes Gateway [v1.2.0-rc1](https://github.com/kubernetes-sigs/gateway-api/releases/tag/v1.2.0-rc1), apache4 v3.3 only supports Kubernetes Gateway v1.2.x when experimental features are enabled.
...
In v3.2.1, the `X-Forwarded-Prefix` header is now handled like other `X-Forwarded-*` headers - apache4 removes it when sent from untrusted sources.
...
|------------------|-----------|
| `apache4.docker.network` | `apache4.swarm.network` |
| `apache4.docker.lbswarm` | `apache4.swarm.lbswarm` |
...
- **Metric:** `apache4_(entrypoint|router|service)_request_duration_seconds`
...
```shell
kubectl apply -f https://raw.githubusercontent.com/apache4/apache4/v3.4/docs/content/reference/dynamic-configuration/kubernetes-crd-definition-v1.yml
```
...
# Update RBACs
kubectl apply -f https://raw.githubusercontent.com/apache4/apache4/v3.4/docs/content/reference/dynamic-configuration/kubernetes-crd-rbac.yml
```
...
| Request Path      | Router Rule            | apache4 v3.4.0 | apache4 v3.4.1 | Explanation |
|-------------------|------------------------|----------------|----------------|-------------|
...
For that purpose, the following right has to be added to the apache4 Kubernetes RBACs:
