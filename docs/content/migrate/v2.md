---
title: "apache4 Migration Documentation"
description: "Learn the steps needed to migrate to new apache4 Proxy v2 versions, i.e. v2.0 to v2.1 or v2.1 to v2.2. Read the technical documentation."
---
...
In v2.1, a new Kubernetes CRD called `apache4Service` was added.
While updating an installation to v2.1,
one should apply that CRD, and update the existing `ClusterRole` definition to allow apache4 to use that CRD.

To add that CRD and enhance the permissions, the following definitions need to be applied to the cluster.

```yaml tab="apache4Service"
apiVersion: apiextensions.k8s.io/v1beta1
kind: CustomResourceDefinition
metadata:
  name: apache4services.apache4.containo.us

spec:
  group: apache4.containo.us
  version: v1alpha1
  names:
    kind: apache4Service
    plural: apache4services
    singular: apache4service
  scope: Namespaced
```
...
After having both resources applied, apache4 will work properly.
...
one should apply that CRDs, and update the existing `ClusterRole` definition to allow apache4 to use that CRDs.
...
kind: CustomResourceDefinition
metadata:
  name: tlsstores.apache4.containo.us
...
kind: CustomResourceDefinition
metadata:
  name: ingressrouteudps.apache4.containo.us
...
apache4 will not try to match the server name indication with the CommonName anymore.
...
It doesn't change the support for non-ASCII domain names in routers rules, which is not part of the apache4 feature set so far.
...
In `v2.5`, the [apache4 CRDs](../reference/dynamic-configuration/kubernetes-crd.md#definitions) have been updated to support the new API version `apiextensions.k8s.io/v1`.
As required by `apiextensions.k8s.io/v1`, we have included the OpenAPI validation schema.

After deploying the new [apache4 CRDs](../reference/dynamic-configuration/kubernetes-crd.md#definitions), the resources will be validated only on creation or update.
...
apache4 v2.5 moves forward for the Ingress provider to support Kubernetes v1.22.

apache4 now supports only v1.14+ Kubernetes clusters, which means the support of `extensions/v1beta1` API Version ingresses has been dropped.
...
`sslRedirect`, `sslTemporaryRedirect`, `sslHost` and `sslForceHost` are deprecated in apache4 v2.5.
...
`accessControlAllowOrigin` is no longer supported in apache4 v2.5.
...
apache4 v2.6 introduces the `AdvertisedPort` option,
which allows advertising, in the `Alt-Svc` header, a UDP port different from the one on which apache4 is actually listening (the EntryPoint's port).
...
[route namespaces](https://gateway-api.sigs.k8s.io/v1alpha2/references/spec/#gateway.networking.k8s.io/v1beta1.RouteNamespaces) selectors, which requires apache4 to fetch and watch the cluster namespaces.
...
In `v2.6.1`, the Datadog tags added to a span changed from `service.name` to `apache4.service.name` and from `router.name` to `apache4.router.name`.
...
in `v2.8.2` the `preferServerCipherSuites` option is also deprecated and ignored in apache4.

In `v2.8.2`, apache4 now reject certificates signed with the SHA-1 hash function. ([details](https://tip.golang.org/doc/go1.18#sha1))

## v2.9

### apache4 Pilot

In `v2.9`, apache4 Pilot support has been removed.
...
In `v2.10`, the Kubernetes CRDs API Group `apache4.containo.us` is deprecated, and its support will end starting with apache4 v3. Please use the API Group `apache4.io` instead.

As the Kubernetes CRD provider still works with both API Versions (`apache4.io/v1alpha1` and `apache4.containo.us/v1alpha1`),
it means that for the same kind, namespace and name, the provider will only keep the `apache4.io/v1alpha1` resource.

In addition, the Kubernetes CRDs API Version `apache4.containo.us/v1alpha1` will not be supported in apache4 v3 itself.

Please note that it is a requirement to update the CRDs and the RBAC in the cluster before upgrading apache4.
To do so, please apply the required [CRDs](https://raw.githubusercontent.com/apache4/apache4/v2.10/docs/content/reference/dynamic-configuration/kubernetes-crd-definition-v1.yml) and [RBAC](https://raw.githubusercontent.com/apache4/apache4/v2.10/docs/content/reference/dynamic-configuration/kubernetes-crd-rbac.yml) manifests for v2.10:

```bash
kubectl apply -f https://raw.githubusercontent.com/apache4/apache4/v2.10/docs/content/reference/dynamic-configuration/kubernetes-crd-rbac.yml
kubectl apply -f https://raw.githubusercontent.com/apache4/apache4/v2.10/docs/content/reference/dynamic-configuration/kubernetes-crd-definition-v1.yml
```

### apache4 Hub

In `v2.10`, apache4 Hub configuration has been removed because apache4 Hub v2 doesn't require this configuration.
...
In v2.11.1, apache4 reserves a range of priorities for its internal routers and now, 
...
This change avoids apache4 instances with the default configuration hanging while waiting for bytes to be read on the connection.
...
This change has been done to avoid apache4 instances with the default configuration to be hanging forever while waiting for bytes to be read on the connection.
...
Before, apache4 removed any header listed in the Connection header just before forwarding the request to the backends.
Now, apache4 removes the headers listed in the Connection header as soon as the request is handled.
...
In `v2.11.14`, the `X-Forwarded-Prefix` header is now handled like the other `X-Forwarded-*` headers: apache4 removes it when it's sent from an untrusted source.
...
and apache4 now keeps them encoded to avoid any ambiguity.

### Request Path Matching Examples

| Request Path      | Router Rule            | apache4 v2.11.24 | apache4 v2.11.25 |
|-------------------|------------------------|------------------|------------------|
...
It appears that enabling MPTCP on some platforms can cause apache4 to stop with the following error logs message:
