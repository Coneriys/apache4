...
Set the `watch` option to `true` to allow apache4 to automatically watch for file changes.
...
!!! warning
    ...
    Templating does not work in the apache4 main static configuration file.

apache4 supports using Go templating to automatically generate repetitive sections of configuration files.
...
??? example "Configuring Using Templating"
    ...
    tls:
      certificates:
      {{ range $i, $e := until 10 }}
      - certFile: "/etc/apache4/cert-{{ $e }}.pem"
        keyFile: "/etc/apache4/cert-{{ $e }}.key"
        store:
        - "my-store-foo-{{ $e }}"
        - "my-store-bar-{{ $e }}"
      {{end}}
