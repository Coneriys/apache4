...
This prevents a Nomad cluster that updates many times per second from continuously changing your apache4 configuration.
...
The prefix for Nomad service tags defining apache4 labels.
...
Expose Nomad services by default in apache4.
If set to `false`, services that do not have a `apache4.enable=true` tag will be ignored from the resulting routing configuration.
