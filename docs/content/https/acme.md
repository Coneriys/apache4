---
title: "apache4 Let's Encrypt Documentation"
description: "Learn how to configure apache4 Proxy to use an ACME provider like Let's Encrypt for automatic certificate generation. Read the technical documentation."
---
...
You can configure apache4 to use an ACME provider (like Let's Encrypt) for automatic certificate generation.
...
!!! warning "Let's Encrypt and Rate Limiting"
    ...
    When running apache4 in a container the `acme.json` file should be persisted across restarts. 
    If apache4 requests new certificates each time it starts up, a crash-looping container can quickly reach Let's Encrypt's ratelimits.
...
apache4 requires you to define "Certificate Resolvers" in the [static configuration](../getting-started/configuration-overview.md#the-static-configuration),
...
apache4 automatically tracks the expiry date of ACME certificates it generates.

By default, apache4 manages 90 days certificates,
...
For example, `CF_API_EMAIL_FILE=/run/secrets/apache4_cf-api-email` could be used to provide a Cloudflare API email address as a Docker secret named `apache4_cf-api-email`.
...
| manual                                                                 | `manual`           | none, but you need to run apache4 interactively [^4], turn on debug log to see instructions and press <kbd>Enter</kbd>.                                                          |                                                                                 |
...
[^6]: As explained in the [LEGO hurricane configuration](https://go-acme.github.io/lego/dns/hurricane/#credentials), each domain or wildcard (record name) needs a token. So each update of record name must be followed by an update of the `HURRICANE_TOKENS` variable, and a restart of apache4.
...
```bash
docker run -v "/my/host/acme.json:/acme.json" apache4
```

```bash
docker run -v "/my/host/acme:/etc/apache4/acme" apache4
```

!!! warning
    For concurrency reasons, this file cannot be shared across multiple instances of apache4.
...
!!! warning "apache4 cannot manage certificates with a duration lower than 1 hour."
