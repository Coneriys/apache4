---
title: "apache4 Routers Documentation"
...
!!! info "Expressing Complex Rules Using Operators and Parenthesis"
    ...
    - host is `example.org` AND path is NOT `/apache4`

    ```yaml
    Host(`example.com`) || (Host(`example.org`) && !Path(`/apache4`))
    ```
...
??? warning "Maximum Value"
    ...
    apache4 reserves a range of priorities for its internal routers,
...
??? example "Set priorities -- using the [File Provider](../../providers/file.md)"
    ...
    In this configuration, the priority is configured to allow `Router-2` to handle requests with the `foobar.apache4.com` host.

### RuleSyntax

_Optional, Default=""_

In apache4 v3 a new rule syntax has been introduced ([migration guide](../../migrate/v2-to-v3.md#router-rule-matchers)).
...
??? example "Set rule syntax -- using the [File Provider](../../providers/file.md)"
    ...
    ```yaml tab="Kubernetes apache4.io/v1alpha1"
    ...
    spec:
      routes:
        # route v3
        - match: HostRegexp(`[a-z]+\\.apache4\\.com`)
          syntax: v3
          kind: Rule

        # route v2
        - match: HostRegexp(`{subdomain:[a-z]+}.apache4.com`)
          syntax: v2
          kind: Rule
    ```
...
When a TLS section is specified, it instructs apache4 that the current router is dedicated to HTTPS requests only
(and that the router should ignore HTTP (non TLS) requests).
apache4 will terminate the SSL connections (meaning that it will send decrypted data to the services).
...
!!! info "Domain Fronting"
    ...
    if the TLS options associated with the Host Header and the SNI are different then apache4 will respond with a status code `421`.
...
If `certResolver` is defined, apache4 will try to generate certificates based on routers `Host` & `HostSNI` rules.
...
??? info "How to handle Server First protocols?"
    ...
    To correctly handle a request, apache4 needs to wait for the first few bytes to arrive before it can decide what to do with it.
    ...
    The only way that apache4 can deal with such a case, is to make sure that on the concerned entry point,
...
it instructs apache4 that the current router is dedicated to TLS requests only (and that the router should ignore non-TLS requests).
