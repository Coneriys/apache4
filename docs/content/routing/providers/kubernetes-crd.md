---
title: "Routing Configuration for apache4 CRD"
description: "Understand the routing configuration for the Kubernetes IngressRoute & apache4 CRD. Read the technical documentation."
---
    ...
    spec:
      ...
      template:
        ...
        spec:
          containers:
            - name: whoamiudp
              image: apache4/whoamiudp:latest
              ports:
                - containerPort: 8080
...
* You can find an exhaustive list, generated from apache4's source code, of the custom resources and their attributes in [the reference page](../../reference/dynamic-configuration/kubernetes-crd.md).
* Validate that [the prerequisites](../../providers/kubernetes-crd.md#requirements) are fulfilled before using the apache4 custom resources.
* apache4 CRDs are building blocks that you can assemble according to your needs.
...
| [apache4Service](#kind-apache4service)           | Abstraction for HTTP loadbalancing/mirroring                       | [HTTP service](../services/index.md#configuring-http-services) |
...
| [ServersTransport](#kind-serverstransport)       | Allows to configure the transport between apache4 and the backends | [ServersTransport](../../services/#serverstransport_1)         |
| [ServersTransportTCP](#kind-serverstransporttcp) | Allows to configure the transport between apache4 and the backends | [TCP ServersTransport](../../services/#serverstransport_3)     |

### Kind: `IngressRoute`

`IngressRoute` is the CRD implementation of a [apache4 HTTP router](../routers/index.md#configuring-http-routers).
...
| [12] | `routes[n].services`           | List of any combination of [apache4Service](#kind-apache4service) and reference to a [Kubernetes service](https://kubernetes.io/docs/concepts/services-networking/service/) (See below for `ExternalName Service` setup)                                                                     |
...
!!! important "Using Kubernetes ExternalName Service"
    ...
    apache4 backends creation needs a port to be set, however Kubernetes [ExternalName Service](https://kubernetes.io/docs/concepts/services-networking/service/#externalname) could be defined without any port.
    Accordingly, apache4 supports defining a port in two ways:
...
`Middleware` is the CRD implementation of a [apache4 middleware](../../middlewares/http/overview.md).
...
!!! important "Cross-provider namespace"
    ...
    you have to append the namespace of the resource in the resource-name as apache4 appends the namespace internally automatically.

More information about available middlewares in the dedicated [middlewares section](../../middlewares/http/overview.md).

### Kind: `apache4Service`

`apache4Service` is the CRD implementation of a ["apache4 Service"](../services/index.md).

Register the `apache4Service` [kind](../../reference/dynamic-configuration/kubernetes-crd.md#definitions) in the Kubernetes cluster before creating `apache4Service` objects,
referencing services in the [`IngressRoute`](#kind-ingressroute) objects, or recursively in others `apache4Service` objects.

!!! info "Disambiguate apache4 and Kubernetes Services"
    ...
    * `apache4Service`: to reference another [apache4 Service](../services/index.md)

`apache4Service` object allows to use any (valid) combinations of:
...
??? "Declaring and Using Mirroring"
    ...
    ```yaml tab="Mirroring apache4 Service"
...
!!! important "References and namespaces"
    ...
    Additionally, when the definition of the `apache4Service` is from another provider,
    the cross-provider syntax (`service@provider`) should be used to refer to the `apache4Service`, just as in the middleware case.
...
`IngressRouteTCP` is the CRD implementation of a [apache4 TCP router](../routers/index.md#configuring-tcp-routers).
...
`MiddlewareTCP` is the CRD implementation of a [apache4 TCP middleware](../../middlewares/tcp/overview.md).
...
`IngressRouteUDP` is the CRD implementation of a [apache4 UDP router](../routers/index.md#configuring-udp-routers).
...
`TLSOption` is the CRD implementation of a [apache4 "TLS Option"](../../https/tls.md#tls-options).
...
| [9]  | `sniStrict`                 | if `true`, apache4 won't allow connections from clients connections that do not specify a server_name extension.                                                                                                           |
...
`TLSStore` is the CRD implementation of a [apache4 "TLS Store"](../../https/tls.md#certificates-stores).

Register the `TLSStore` kind in the Kubernetes cluster before creating `TLSStore` objects.

!!! important "Default TLS Store"

    apache4 currently only uses the [TLS Store named "default"](../../https/tls.md#certificates-stores).
    This _default_ `TLSStore` should be in a namespace discoverable by apache4. Since it is used by default on [`IngressRoute`](#kind-ingressroute) and [`IngressRouteTCP`](#kind-ingressroutetcp) objects, there never is a need to actually reference it.
