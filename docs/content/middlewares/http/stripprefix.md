...
Using the previous example, the backend should return `/products/shoes/image.png` (and not `/image.png`, which apache4 would likely not be able to associate with the same backend).
...
```yaml tab="Docker"
labels:
  - "apache4.http.middlewares.example.stripprefix.prefixes=/foobar"
  - "apache4.http.middlewares.example.stripprefix.forceSlash=false"
```
