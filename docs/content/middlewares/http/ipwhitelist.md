---
title: "apache4 HTTP Middlewares IPWhiteList"
description: "Learn how to use IPWhiteList in HTTP middleware for limiting clients to specific IPs in apache4 Proxy. Read the technical documentation."
---
...
labels:
  - "apache4.http.middlewares.test-ipwhitelist.ipwhitelist.sourcerange=127.0.0.1/32, 192.168.1.7"
```
...
The `ipStrategy` option defines two parameters that set how apache4 determines the client IP: `depth`, and `excludedIPs`.  
...
The `depth` option tells apache4 to use the `X-Forwarded-For` header and take the IP located at the `depth` position (starting from the right).
...
labels:
  - "apache4.http.middlewares.test-ipwhitelist.ipwhitelist.sourcerange=127.0.0.1/32, 192.168.1.7"
  - "apache4.http.middlewares.test-ipwhitelist.ipwhitelist.ipstrategy.depth=2"
```
...
labels:
    - "apache4.http.middlewares.test-ipwhitelist.ipwhitelist.sourceRange=127.0.0.1/32, 192.168.1.0/24"
    - "apache4.http.middlewares.test-ipwhitelist.ipwhitelist.ipstrategy.excludedips=127.0.0.1/32, 192.168.1.7"
```
...
labels:
  - "apache4.http.middlewares.test-ipWhiteList.ipWhiteList.sourcecriterion.ipstrategy.ipv6Subnet=64"
```
