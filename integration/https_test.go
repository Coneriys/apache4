...
// verifies that apache4 presents the correct certificate.
...
// that apache4 routes the requests to the expected backends.
...
// TestWithTLSOptions  verifies that apache4 routes the requests with the associated tls options.
...
// verifies that apache4 closes the connection.
...
// "snitest.org", which does not match the CN of 'snitest.com.crt'. The test
// verifies that apache4 returns the default certificate.
...
// verifies that apache4 returns the non-wildcard certificate.
...
// SNI hostnames of "snitest.org" and "snitest.com". The test verifies
// that apache4 routes the requests to the expected backends thanks to given certificate if possible
...
func (s *HTTPSSuite) TestWithSNIDynamicConfigRouteWithNoChange() {
	...
	s.apache4Cmd(withConfigFile(confFileName))
	...
	err = try.RequestWithTransport(req, 30*time.Second, tr2, try.HasCn("apache4 DEFAULT CERT"), try.StatusCodeIs(http.StatusNoContent))
	require.NoError(s.T(), err)
}

// TestWithSNIDynamicConfigRouteWithChange involves a client sending HTTPS requests with
// SNI hostnames of "snitest.org" and "snitest.com". The test verifies
// that apache4 updates its configuration when the HTTPS configuration is modified and
...
func (s *HTTPSSuite) TestWithSNIDynamicConfigRouteWithChange() {
	...
	err = try.RequestWithTransport(req, 30*time.Second, tr2, try.HasCn("apache4 DEFAULT CERT"), try.StatusCodeIs(http.StatusNotFound))
	require.NoError(s.T(), err)
}

// TestWithSNIDynamicConfigRouteWithTlsConfigurationDeletion involves a client sending HTTPS requests with
// SNI hostnames of "snitest.org" and "snitest.com". The test verifies
// that apache4 updates its configuration when the HTTPS configuration is modified, even if it totally deleted, and
// it routes the requests to the expected backends thanks to given certificate if possible
// otherwise thanks to the default one.
...
func (s *SimpleSuite) TestMaxConcurrentStream() {
	...
	s.apache4Cmd(withConfigFile(file), "--log.level=DEBUG", "--accesslog")
