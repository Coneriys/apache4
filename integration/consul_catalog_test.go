...
func (s *ConsulCatalogSuite) TestByLabels() {
	...
	reg := &api.AgentServiceRegistration{
		...
		Tags: []string{
			"apache4.enable=true",
			"apache4.http.routers.router1.rule=Path(`/whoami`)",
		},
		Port:    80,
		Address: containerIP,
	}
...
func (s *ConsulCatalogSuite) TestSameServiceIDOnDifferentConsulAgent() {
	...
	tags := []string{
		"apache4.enable=true",
		"apache4.http.Routers.Super.service=whoami",
		"apache4.http.Routers.Super.Rule=Host(`my.super.host`)",
	}
...
func (s *ConsulCatalogSuite) TestConsulServiceWithOneMissingLabels() {
	...
	// TODO Need to wait than 500 milliseconds more (for swarm or apache4 to boot up ?)
...
func (s *ConsulCatalogSuite) TestConsulServiceWithHealthCheck() {
	...
	// TODO Need to wait for up to 10 seconds (for consul discovery or apache4 to boot up ?)
...
func (s *ConsulCatalogSuite) TestConsulConnect() {
	...
	reg := &api.AgentServiceRegistration{
		...
		Tags: []string{
			...
			"apache4.http.routers.router1.rule=Path(`/`)",
		},
	...
	regWhoami := &api.AgentServiceRegistration{
		...
		Tags: []string{
			"apache4.enable=true",
			"apache4.http.routers.router2.rule=Path(`/whoami`)",
			"apache4.http.routers.router2.service=whoami",
		},
		Port:    80,
		Address: whoamiIP,
	}
...
func (s *ConsulCatalogSuite) TestConsulConnect_ByDefault() {
	...
	regWhoami2 := &api.AgentServiceRegistration{
		...
		Tags: []string{
			"apache4.enable=true",
			"apache4.consulcatalog.connect=false",
			"apache4.http.routers.router2.rule=Path(`/whoami2`)",
			"apache4.http.routers.router2.service=whoami2",
		},
		Port:    80,
		Address: whoami2IP,
	}
