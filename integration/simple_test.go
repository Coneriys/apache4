...
func (s *SimpleSuite) TestInvalidConfigShouldFail() {
	_, output := s.cmdapache4(withConfigFile("fixtures/invalid_configuration.toml"))
...
func (s *SimpleSuite) TestSimpleDefaultConfig() {
	s.cmdapache4(withConfigFile("fixtures/simple_default.toml"))
...
func (s *SimpleSuite) TestSimpleFastProxy() {
	...
	srv1 := httptest.NewServer(http.HandlerFunc(func(rw http.ResponseWriter, req *http.Request) {
		assert.Contains(s.T(), req.Header, "X-apache4-Fast-Proxy")
		callCount++
	}))
	defer srv1.Close()
	...
	s.apache4Cmd(withConfigFile(file), "--log.level=DEBUG")
...
func (s *SimpleSuite) TestWithWebConfig() {
	s.cmdapache4(withConfigFile("fixtures/simple_web.toml"))
...
func (s *SimpleSuite) TestRequestAcceptGraceTimeout() {
	...
	cmd, _ := s.cmdapache4(withConfigFile(file))
	...
	// Send SIGTERM to apache4.
	proc, err := os.FindProcess(cmd.Process.Pid)
	require.NoError(s.T(), err)
	err = proc.Signal(syscall.SIGTERM)
	require.NoError(s.T(), err)

	// Give apache4 time to process the SIGTERM and send a request half-way
	...
	// Expect apache4 to shut down gracefully once the request accepting grace
	// period has elapsed.
	...
	case <-time.After(10 * time.Second):
		...
		// this point.
		s.T().Fatal("apache4 did not terminate in time")
	}
}
...
func (s *SimpleSuite) TestDefaultEntryPointHTTP() {
	...
	s.apache4Cmd("--entryPoints.http.Address=:8000", "--log.level=DEBUG", "--providers.docker", "--api.insecure")
...
func (s *SimpleSuite) TestMetricsPrometheusDefaultEntryPoint() {
	...
	s.apache4Cmd("--entryPoints.http.Address=:8000", "--api.insecure", "--metrics.prometheus.buckets=0.1,0.3,1.2,5.0", "--providers.docker", "--metrics.prometheus.addrouterslabels=true", "--log.level=DEBUG")
...
func (s *SimpleSuite) TestMetricsPrometheusTwoRoutersOneService() {
	...
	s.apache4Cmd("--entryPoints.http.Address=:8000", "--api.insecure", "--metrics.prometheus.buckets=0.1,0.3,1.2,5.0", "--providers.docker", "--metrics.prometheus.addentrypointslabels=false", "--metrics.prometheus.addrouterslabels=true", "--log.level=DEBUG")
	...
	for range 10 {
		...
		assert.Contains(s.T(), string(body), "apache4_router_requests_total{code=\"200\",method=\"GET\",protocol=\"http\",router=\"router1@docker\",service=\"whoami1@docker\"} 1")
		assert.Contains(s.T(), string(body), "apache4_router_requests_total{code=\"200\",method=\"GET\",protocol=\"http\",router=\"router2@docker\",service=\"whoami1@docker\"} 1")
		// Reqs count of 2 for service behind both routers
		assert.Contains(s.T(), string(body), "apache4_service_requests_total{code=\"200\",method=\"GET\",protocol=\"http\",service=\"whoami1@docker\"} 2")
	}
}
...
func (s *SimpleSuite) TestMetricsWithBufferingMiddleware() {
	...
	assert.Contains(s.T(), string(body), "apache4_entrypoint_requests_total{code=\"200\",entrypoint=\"webA\",method=\"GET\",protocol=\"http\"} 1")
	assert.Contains(s.T(), string(body), "apache4_entrypoint_requests_bytes_total{code=\"200\",entrypoint=\"webA\",method=\"GET\",protocol=\"http\"} 0")
	assert.Contains(s.T(), string(body), "apache4_entrypoint_responses_bytes_total{code=\"200\",entrypoint=\"webA\",method=\"GET\",protocol=\"http\"} 31")

	assert.Contains(s.T(), string(body), "apache4_service_requests_total{code=\"200\",method=\"GET\",protocol=\"http\",service=\"service-without@file\"} 1")
	assert.Contains(s.T(), string(body), "apache4_service_requests_bytes_total{code=\"200\",method=\"GET\",protocol=\"http\",service=\"service-without@file\"} 0")
	assert.Contains(s.T(), string(body), "apache4_service_responses_bytes_total{code=\"200\",method=\"GET\",protocol=\"http\",service=\"service-without@file\"} 31")

	// For forbidden requests, the entrypoints have metrics, the services don't.
	assert.Contains(s.T(), string(body), "apache4_entrypoint_requests_total{code=\"413\",entrypoint=\"webB\",method=\"GET\",protocol=\"http\"} 1")
	assert.Contains(s.T(), string(body), "apache4_entrypoint_requests_bytes_total{code=\"413\",entrypoint=\"webB\",method=\"GET\",protocol=\"http\"} 0")
	assert.Contains(s.T(), string(body), "apache4_entrypoint_responses_bytes_total{code=\"413\",entrypoint=\"webB\",method=\"GET\",protocol=\"http\"} 24")

	// For disallowed responses, the entrypoint and service metrics don't have the same status code.
	assert.Contains(s.T(), string(body), "apache4_entrypoint_requests_bytes_total{code=\"500\",entrypoint=\"webC\",method=\"GET\",protocol=\"http\"} 0")
	assert.Contains(s.T(), string(body), "apache4_entrypoint_requests_total{code=\"500\",entrypoint=\"webC\",method=\"GET\",protocol=\"http\"} 1")
	assert.Contains(s.T(), string(body), "apache4_entrypoint_responses_bytes_total{code=\"500\",entrypoint=\"webC\",method=\"GET\",protocol=\"http\"} 21")

	assert.Contains(s.T(), string(body), "apache4_service_requests_bytes_total{code=\"200\",method=\"GET\",protocol=\"http\",service=\"service-resp@file\"} 0")
	assert.Contains(s.T(), string(body), "apache4_service_requests_total{code=\"200\",method=\"GET\",protocol=\"http\",service=\"service-resp@file\"} 1")
	assert.Contains(s.T(), string(body), "apache4_service_responses_bytes_total{code=\"200\",method=\"GET\",protocol=\"http\",service=\"service-resp@file\"} 31")
}
...
func (s *SimpleSuite) TestIPStrategyAllowlist() {
	...
	s.apache4Cmd(withConfigFile("fixtures/simple_allowlist.toml"))
...
func (s *SimpleSuite) TestIPStrategyWhitelist() {
	...
	s.apache4Cmd(withConfigFile("fixtures/simple_whitelist.toml"))
...
func (s *SimpleSuite) TestDebugLog() {
	...
	_, output := s.cmdapache4(withConfigFile(file))
	...
	if regexp.MustCompile("ThisIsABearerToken").MatchReader(output) {
		log.Info().Msgf("apache4 Logs: %s", output.String())
		log.Info().Msg("Found Authorization Header in apache4 DEBUG logs")
		s.T().Fail()
	}
}
...
func (s *SimpleSuite) TestDenyFragment() {
	...
	s.apache4Cmd(withConfigFile("fixtures/simple_default.toml"))
...
func (s *SimpleSuite) TestMaxHeaderBytes() {
	...
	// The test server and apache4 config file both specify a max request header size of 1.25 MB.
